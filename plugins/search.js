    ...
    try {
      ...
      if (!match)
        return message.reply(
          `Give me a user name like ${prefix}github Limule3650`
        );
  ...
  async (message, text, { cmdName }) => {
    ...
    try {
      ...
      const { thumb, lyrics, title, artist } = res.result,
        ...
        contextInfo = {
          externalAdReply: {
            ...(await message.bot.contextInfo("𝗩𝗘𝗟𝗗𝗥𝗔-𝗠𝗗", `Lyrics-${text}`)),
          },
        };
      await send(
        message,
...
smd(
  ...
  async (message, match) => {
    try {
      if (!match) return message.reply("Give me package name.📦");
...
smd(
  ...
  async (message, text) => {
    if (!text)
      return await message.reply(
        "Give Me Number without + sign. Example: .nowa 2376985819xx"
      );
    const inputNumber = text.split(" ")[0];
    if (!inputNumber.includes("x"))
      return message.reply(
        `*You did not add x in number.*\nExample: ${prefix}nowa 2376985819xx  \n ${Config.caption}`
      );
    message.reply(
